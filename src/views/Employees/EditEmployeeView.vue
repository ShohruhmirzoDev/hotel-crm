...
const ApiCallEditEmployee = ()=>{

    axios.get(`/api/Staff/get-by-id ${editId}`)
    .then((res)=>{
        if (res.status == 200) {
            ...
            FatherName.value = res.data.fatherName
            Position.value = res.data.positionId
            Phone.value = res.data.phoneNumber
            Birthday.value = res.data.birthDate
...
const EditStaffAdd = ()=>{
  if (UserName.value == null ||
    ...
    axios.put("/api/Staff/update-staff",{
      id: editId,
